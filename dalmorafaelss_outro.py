...
scene.camera.pos = vec(50, 40, 50)
scene.camera.axis = vec(-50, -40, -50)
...
class Carro:
    def __init__(self, eixo, direcao):
        ...
        self.speed = (15 + random()*15) * direcao
...
class Cidade:
    def __init__(self):
        ...
        box(pos=vec(0, 0.01, 0), size=vec(14, 0.1, 140), color=vec(0.2, 0.2, 0.2))
    ...
    def construir(self):
        ...
        for x in range(-55, 60, 18):
            for z in range(-55, 60, 18):
                if abs(x) > 12 and abs(z) > 12 and random() > 0.25:
                    self.add_predio(x, z)
    ...
    def add_predio(self, x, z):
        h = 8 + random()*16
        w = 5 + random()*4
        cor = vec(random()*0.5, random()*0.5, random()*0.5 + 0.2)
...
class Estacao:
    ...
    def gerar_chuva(self):
        ...
        if len(self.gotas) < 150:
            g = sphere(pos=vec(random()*100-50, 40+random()*10, random()*100-50), 
        ...
        for g in self.gotas:
            g.pos += g.v * 0.05
            if g.pos.y < 0: g.pos.y = 40 + random()*10
            g.visible = True
...
# Frota de carros
carros = []
for _ in range(4): carros.append(Carro('x', 1))
for _ in range(4): carros.append(Carro('x', -1))
for _ in range(4): carros.append(Carro('z', 1))
for _ in range(4): carros.append(Carro('z', -1))
